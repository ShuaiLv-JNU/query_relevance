# 1.阿里天池比赛简介
- 名称：医学搜索Query相关性判断
- 地址：https://tianchi.aliyun.com/competition/entrance/532001
- Rank：76/23141
- score：0.867
...
> - 引入一些"噪声"(不满足传递性的样本)可能会使模型更加"鲁棒",不那么依赖于训练数据中的特定模式。
> - 如果我们对所有类别的样本都使用同样的增强比例,那么增强后的数据集中,不同类别样本数量的差距会进一步拉大。这可能导致模型更偏向于数量占优的类别。
...
### 8.1 数据增强
```
# 运行data_augment.py文件，进行数据扩充
> python data_augment.py
```

### 8.2 训练
```
# 有三个训练文件，可以随意选择，以run_ernie.py为例, 进行模型训练
> python run_ernie.py
```
